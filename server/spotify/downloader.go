...
func downloadYTaudio(id, path, filePath string) error {
    ...
    if _, err := os.Stat(path); os.IsNotExist(err) {
        if mkErr := os.MkdirAll(path, 0755); mkErr != nil {
            return fmt.Errorf("failed to create download directory: %v", mkErr)
        }
    }
    ...
    // yt-dlp command: extract audio, convert to wav/m4a, output to target file
    cmd := exec.Command(
        "yt-dlp",
        "--no-playlist",
        "-x", "--audio-format", "m4a",
    ...
    if err := cmd.Run(); err != nil {
        logger.Error("yt-dlp download failed", slog.Any("error", err))
        return fmt.Errorf("yt-dlp download failed: %v", err)
    }

    // Sanity check: ensure file exists and has size > 0
    fi, err := os.Stat(filePath)
    if err != nil || fi.Size() == 0 {
        return fmt.Errorf("download failed or file empty: %s", filePath)
    }
